...
const getAvailableTimeRangesForExceptions = (exceptions, timeZone) => {
  const nextBoundary = findNextBoundary(timeZone, TODAY); // replaced timeZone with Europe/London at first
  const lastBoundary = endOfAvailabilityExceptionRange(timeZone, TODAY); // replaced timeZone with Europe/London at first
...
const onExceptionStartDateChange = (value, timeRanges, props) => {
  // TODO: fix UI bug when selecting 'not available' exception then date and not able to set the time or do anything 
  // Currently need to select date first, then what you want to do with it from radio buttons (not available) 
